...
struct pool{
	...
	> struct ptr_d{
		...
		typedef ptrdiff_t difference_type;
		typedef random_access_iterator_tag iterator_category;
		ptr_d(POOL_PTR pool_ptr,INDEX index):pool_ptr(pool_ptr),index(index){}
		template<
...
> using persistent_allocator_managed=pool::allocator<INDEX,_PAYLOAD_,pool::POOL_ALLOCATOR,pool::template mmap_allocator<_PAYLOAD_>,uint8_t>;
template<
	typename _PAYLOAD_,
	typename INDEX=uint8_t
> using persistent_allocator_unmanaged=pool::allocator<INDEX,_PAYLOAD_,pool::POOL_ALLOCATOR,pool::template mmap_allocator<_PAYLOAD_>,void>;
template<
	typename _PAYLOAD_,
	typename INDEX=uint8_t
> using volatile_allocator_managed=pool::allocator<INDEX,_PAYLOAD_,pool::POOL_ALLOCATOR,std::allocator<char>,uint8_t>;
template<
	typename _PAYLOAD_,
	typename INDEX=uint8_t
> using volatile_allocator_unmanaged=pool::allocator<INDEX,_PAYLOAD_,pool::POOL_ALLOCATOR,std::allocator<char>,void>;
template<
	typename _PAYLOAD_
